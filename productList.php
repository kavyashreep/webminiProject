...
<html>
		...
		<style>
			* {box-sizing: border-box}
			body {
				...
				height:600px;				
				margin:0;
				background-image: url("ProjectImages/ProductList.png");
	...
	<body>
		<?php
				...
				$conn = OpenCon();
				$tname=$_POST["search"];
				$sql="SELECT item_id,item_name,item_quant,item_brand,MRP,offer,item_price,
				      DATE_FORMAT(Best_before, '%M %D %Y') as bdate,food_type,ratings,stock,
					  item_image,direction,direction_image FROM $tname";
				...
				if($result)
				{
					if ($result->num_rows > 0) {
						while($row = $result->fetch_array())
						{
							...
							$dir_img_arr[$item_count]=$row['direction_image'];
				...
				if($result){
					...
					//		echo "<div class='numbertext'>1/". ($itemCount+1)."</div>";
							...
							echo "<tr><td>by"." "."<i style='color:blue'>". $row['item_brand'] . "</i></td></tr>";
							echo "<tr><td>M.R.P:"." "."<del><i class='fa fa-inr'></i>". $row['MRP'] . "</del></td></tr>";
							echo "<tr rowspan='2'><td style='color:red'>Sale:"." "."<i class='fa fa-inr'></i>".$row['item_price'] ."</td></tr>";
							echo "<tr><td>You save:<b style='color:red'><i class='fa fa-inr'></i>".($row['MRP']-$row['item_price'])."(".$row['offer']."%)</b></td></tr>";
							...
							if($row['food_type']=="Vegetarian"){
								echo "<tr><td style='color:green'>".$row['food_type']."</td></tr>";
							}
							else{
								echo "<tr><td style='color:red'>".$row['food_type']."</td></tr>";
							}	
							echo "</table>";
							echo "</div>";
							...
							echo "<script language='javascript'>
								...
								function testDir(a)
								{
									...
									var pro_dir_img = ".(json_encode($dir_img_arr)).";
									console.log(pro_dir[a]);
									window.location.href='Itemdirection.php?dir='+pro_dir[a]+'&img='+pro_dir_img[a];
								}";
							echo '</script>';
